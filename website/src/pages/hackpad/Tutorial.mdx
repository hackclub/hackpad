...
Add another rectangle, this time with it's start starting on the top line of our rectangle. Set it's width to 18.5mm and height to 31mm. After that use the distance tool to set the distance from the left of the inner rectangle to the left edge of the outer rectangle to x_distance_in_kicad+5.75mm
